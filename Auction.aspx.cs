...
public partial class Auction : System.Web.UI.Page
{
    ...
    protected string style; public string bigTitle;
    ProductDetails p;
    ...
    {
        int orgid = orgs().orgID;// Convert.ToInt16(messages.orgID);
        int catid = item().catID;// Convert.ToInt16(messages.catID);
        int itemid = item().ID;// Convert.ToInt16(messages.itemID);
        string bidtype = messages.bidtype;
        int buyerid = custs().ID;// mes
    ...
    protected void build(string prm, string soldFixedBid)
    {
      ...
      //  String soldFixedBid = "bid";
        ...
        int orgid = orgs().ID;// Convert.ToInt16(messages.orgID);
        int catid = item().catID;// Convert.ToInt16(messages.catID);
        int itemid = item().ID;// Convert.ToInt16(messages.itemID);
        string bidtype = messages.bidtype;
        int buyerid = item().ID;// messages.buyerID;
        DateTime biddate = DateTime.Now;
        Decimal bidamount = txtBidamount.Text.Trim() == "" ? 0 : Convert.ToDecimal(txtBidamount.Text);
        int credits = 1;
        int fee_per_bid = 0;
    ...
    protected void bidValues(DataRow dr)
    {
      ...
      //  DataTable dt = new DataTable();
       ...
       /// HtmlGenericControl Description = (HtmlGenericControl)this.FindControl("Description");
       ...
       ///// lblHandling.InnerHtml = items().Handling.ToString();
     ...
     String handlingTime = dr["handlingTime"].ToString() == string.Empty ? "" :  dr["handlingTime"].ToString();
        ...
        String [] itemsIncluded =      dr["itemsIncluded"].ToString().Split( ',')   ;
        ...
        //HtmlGenericControl iteminc1 = (HtmlGenericControl)this.FindControl("iteminc1");
        //HtmlGenericControl iteminc2 = (HtmlGenericControl)this.FindControl("iteminc2");
        //HtmlGenericControl iteminc3 = (HtmlGenericControl)this.FindControl("iteminc3");

        //HtmlGenericControl iteminc4 = (HtmlGenericControl)this.FindControl("iteminc4");

        //HtmlGenericControl iteminc5 = (HtmlGenericControl)this.FindControl("iteminc5");

        iteminc1.InnerHtml = itemsIncluded.Length ==0 ?  "" : itemsIncluded[0];
        iteminc2.InnerHtml = itemsIncluded.Length<=1 ? "" : itemsIncluded[1];
        iteminc3.InnerHtml = itemsIncluded.Length<=2 ? "": itemsIncluded[2];
        iteminc4.InnerHtml = itemsIncluded.Length<=3 ? "": itemsIncluded[3];
        iteminc5.InnerHtml = itemsIncluded.Length<=4  ? ""  : itemsIncluded[4];
       // iteminc6.InnerHtml = itemsIncluded[5] == "" ? "" : itemsIncluded[5];
      ...
      //  lblCondition.Text = condition;
       ...
       // lblHandling.Text = handlingTime.ToString();
        ...
        // HtmlInputControl lblShipping = (HtmlInputControl)this.FindControl("lblShipping");
